<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-semibold text-slate-900 mb-1">{{ dashboardTitle }}</h1>
      <p class="text-slate-600">{{ dashboardDesc }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <canvas ref="progressChartRef" height="120"></canvas>
      </div>

      <!-- Budget Chart -->
      <div class="bg-white rounded-lg border border-slate-200 p-6">
        <h3 class="text-lg font-medium text-slate-900 mb-4">Budget Utilization (KES Millions)</h3>
        <canvas ref="budgetChartRef" height="120"></canvas>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-slate-200 p-6">
      ...
      <div class="bg-slate-100 rounded-lg h-80 flex items-center justify-center relative overflow-hidden">
        ...
        <div class="relative z-10 grid grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-4xl px-8">
          <RouterLink
            v-for="project in userProjects"
            :key="project.id"
            :to="`/dashboard/projects/${project.id}`"
            class="bg-white p-4 rounded-lg shadow-sm border border-slate-200 hover:shadow-md transition-shadow"
          >
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-slate-50 border-b border-slate-200">
            <tr>
              <th class="px-6 py-3 text-left text-sm text-slate-700">Project Name</th>
              <th class="px-6 py-3 text-left text-sm text-slate-700">Ward</th>
              <th class="px-6 py-3 text-left text-sm text-slate-700">Contractor</th>
              <th class="px-6 py-3 text-left text-sm text-slate-700">Status</th>
              <th class="px-6 py-3 text-left text-sm text-slate-700">% Complete</th>
              <th class="px-6 py-3 text-left text-sm text-slate-700">Budget vs Spent</th>
          ...
          <tbody class="divide-y divide-slate-200">
            ...
            >
              ...
              <td class="px-6 py-4 text-sm text-slate-600">{{ project.contractor }}</td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-sm text-slate-600">{{ formatDate(project.lastUpdated) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const onTrackCount = computed(() => projects.filter(p => p.status === 'on-track').length)
const slowProgressCount = computed(() => projects.filter(p => p.status === 'slow-progress').length)
const behindScheduleCount = computed(() => projects.filter(p => p.status === 'behind-schedule').length)
